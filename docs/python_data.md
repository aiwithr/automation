...
### উদাহরণ কোড: REST API দিয়ে সিসকো ডিভাইসের হোস্টনেম নেয়া: বিস্তারিত ব্যাখ্যা
...
#### চলুন CLI থেকে তুলনা করে বুঝি

আপনি যখন CLI-তে কাজ করেন:
```bash
enable
show hostname
```
এভাবে কমান্ড দেন, তাই না? 

এখন একই কাজ API দিয়ে করব। তবে এখানে কয়েকটা জিনিস আলাদা:

1. **অথেনটিকেশন**: CLI-তে enable পাসওয়ার্ড দেন। API-তে ইউজারনেম-পাসওয়ার্ড দুটোই লাগে:
```python
USER = 'admin'
PASS = 'C1sco12345'
```

2. **কোথায় যাব**: CLI-তে ডিভাইসের কনসোলে যান। API-তে URI (ঠিকানা) লাগে:
```python
uri = 'https://devnetsandboxiosxe.cisco.com:443/restconf/data/Cisco-IOS-XE-native:native/hostname'
```
এখানে:
- `https://` = সিকিউর কানেকশন
- `devnetsandboxiosxe.cisco.com` = ডিভাইসের নাম
- `443` = HTTPS পোর্ট
- `/restconf/data/` = REST API এর পথ
- `Cisco-IOS-XE-native:native/hostname` = কী তথ্য চাই
- এখানে আমাদের Cisco IOS XE রাউটার লাগবে

3. **ডাটা ফরম্যাট**: CLI-তে টেক্সট আউটপুট পান। API-তে JSON ফরম্যাট চাই:
```python
headers = {
    "Accept" : "application/yang-data+json"
}
...
4. **কমান্ড পাঠানো**: CLI-তে কমান্ড টাইপ করেন। API-তে `requests` লাইব্রেরি ব্যবহার করে কল করি:
```python
response = requests.get(
    url=uri,          # কোথায় যাব
    headers=headers,  # কী ফরম্যাটে চাই
    auth=(USER, PASS),# ক্রেডেনশিয়াল
    verify=False)     # SSL চেক বন্ধ
```
